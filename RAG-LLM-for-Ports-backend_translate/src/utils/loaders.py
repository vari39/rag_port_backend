...
from .multilingual_processor import EnhancedMultilingualProcessor, create_enhanced_processor
...
class EnhancedMultilingualDocumentLoader:
    """
    Enhanced multilingual document loader with OCR support, robust language detection,
    ...
    def __init__(self, 
        ...
        """
        Initialize the enhanced multilingual document loader.
        ...
        logger.info("Enhanced multilingual document loader initialized")
    
    def load_pdf_documents(self, 
                          pdf_directory: str,
                          recursive: bool = True,
                          auto_translate: bool = True) -> List[Document]:
        """
        Load PDF documents from a directory with enhanced multilingual support.
        
        Args:
            pdf_directory: Directory containing PDF files
            recursive: Whether to search recursively
            auto_translate: Whether to automatically translate to English
        ...
        try:
            ...
            results = self.processor.process_documents_batch(
    ...
    def load_documents_from_files(self, 
                                 ...
                                 auto_translate: bool = True) -> List[Document]:
        """
        Load documents from a list of file paths with enhanced multilingual support.
        ...
        try:
            ...
            # Process documents in batch
            results = self.processor.process_documents_batch(file_paths, auto_translate)
    ...
    def get_processing_statistics(self, pdf_directory: str, recursive: bool = True) -> Dict[str, Any]:
        """Get processing statistics for a directory of documents"""
        try:
            pdf_path = Path(pdf_directory)
            if recursive:
                pdf_files = list(pdf_path.rglob("*.pdf"))
            ...
            results = self.processor.process_documents_batch([str(f) for f in pdf_files])
            return self.processor.get_processing_statistics(results)
...
def create_document_loader(chunk_size: int = 1000, 
                          ...
                          enhanced: bool = True,
                          enable_ocr: bool = True) -> Union[EnhancedMultilingualDocumentLoader, MultilingualDocumentLoader, RecursiveCharacterTextSplitter]:
    """
    ...
    Args:
        ...
        enhanced: Whether to use enhanced multilingual processor
        enable_ocr: Whether to enable OCR support (only for enhanced loader)
    ...
    if multilingual:
        if enhanced:
            return EnhancedMultilingualDocumentLoader(
                chunk_size=chunk_size,
                chunk_overlap=chunk_overlap,
                enable_ocr=enable_ocr
            )
