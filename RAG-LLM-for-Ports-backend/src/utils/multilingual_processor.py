...
class EnhancedMultilingualProcessor:
    ...
    def __init__(
        ...
        if self.enable_ocr:
            ...
            if self.ocr_reader is None:
                ...
                logger.warning("OCR initialization failed. OCR disabled.")
        else:
    ...
    def _initialize_translation_models(self, main_model):
        ...
        pytorch_too_old = False
        try:
            major, minor = map(int, torch.__version__.split(".")[:2])
            if major < 2 or (major == 2 and minor < 6):
                pytorch_too_old = True
                logger.warning(f"PyTorch {torch.__version__} is outdated for translation models. Translation will be disabled.")
        except Exception:
            pass

        # Skip loading translation models if PyTorch is too old
        if pytorch_too_old:
            logger.info("Translation models disabled due to PyTorch version. System will work without translation.")
            models["main"] = None
            return models
        ...
        # Additional pairs (using correct model identifiers)
        additional = {
            "en-es": "Helsinki-NLP/opus-mt-en-es",
            "en-fr": "Helsinki-NLP/opus-mt-en-fr",
            "en-de": "Helsinki-NLP/opus-mt-en-de",
            "en-zh": "Helsinki-NLP/opus-mt-en-zh",  # Fixed: use 'zh' not 'zho'
    ...
    def _initialize_ocr(self):
        try:
            ...
            # EasyOCR uses 'ch_sim' and 'ch_tra' for Chinese, not 'zh'
            ocr_lang_map = {
                "en": "en",
                "es": "es",
                "fr": "fr",
            ...
            for lang in self.ocr_languages:
                mapped = ocr_lang_map.get(lang, lang)
            ...
            logger.info(f"OCR initialized with languages: {easyocr_langs}")
            return reader
        except Exception as e:
            logger.warning(f"Failed to initialize OCR: {e}. OCR will be disabled.")
            return None
    ...
    def translate_text(self, text: str, src: str, tgt: str = "en"):
        ...
        # If no translation models available, return original text
        if not self.translation_models or not any(self.translation_models.values()):
            logger.debug(f"Translation not available. Returning original text.")
            return text, "translation_unavailable"
        ...
        try:
            ...
            elif self.translation_models.get("main"):
                model = self.translation_models["main"]

            if not model:
                logger.debug(f"No translation model available for {key}. Returning original text.")
