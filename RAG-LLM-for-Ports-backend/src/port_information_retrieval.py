...
# Note: If using langchain >= 0.1.0, may need: from langchain.chains.retrieval_qa.base import RetrievalQA
...
class PortInformationRetrieval:
    ...
    async def add_documents_to_vector_store(self, documents: List[Document]) -> bool:
        ...
        try:
            ...
            # Filter complex metadata (ChromaDB only accepts simple types)
            from langchain_community.vectorstores.utils import filter_complex_metadata
            
            # Filter metadata for each document
            filtered_documents = []
            for doc in documents:
                # Create a copy with filtered metadata
                filtered_metadata = {}
                for key, value in doc.metadata.items():
                    # Only keep simple types (str, int, float, bool, None)
            ...
            # Add documents to ChromaDB in batches (ChromaDB has a max batch size limit)
            ...
            logger.info(f"Successfully added {len(filtered_documents)} documents to vector store in {total_batches} batches")
            return True
    ...
    async def query_port_information(self, 
        ...
        try:
            ...
            if not relevant_docs:
                raise ValueError(
                    f"No relevant documents found for the question: '{question}'. "
                    "The system does not have sufficient information in the database to answer this query. "
                    "Please ensure relevant documents have been uploaded and indexed."
                )
