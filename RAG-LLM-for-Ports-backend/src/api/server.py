...
# Load environment variables - try main folder first, then local
main_env = Path(__file__).parent.parent.parent / ".env"
if main_env.exists():
    load_dotenv(main_env)
else:
    load_dotenv()  # Fall back to local .env
...
async def ask_question(request: QueryRequest):
    ...
    try:
        ...
        if not answer or answer.strip() == "":
            raise HTTPException(
                status_code=404, 
                detail="Unable to generate an answer. The system could not find relevant information to answer this question."
            )
...
async def ask_with_workflow(request: WorkflowRequest):
    ...
    try:
        ...
        if not recommendation or recommendation.strip() == "":
            error_msg = "Unable to generate a recommendation. The workflow could not produce a valid result."
...
async def get_system_status():
    ...
    try:
        ...
        from ..utils.schemas import SystemStatus as SystemStatusEnum
        
        if all(components.values()):
            status_value = SystemStatusEnum.HEALTHY
        elif any(components.values()):
            status_value = SystemStatusEnum.DEGRADED
        else:
            status_value = SystemStatusEnum.UNHEALTHY
